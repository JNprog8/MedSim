document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadEncounterHistory(encounterId) {
        try {
            ...
            console.error('Failed to load encounter history:', e);
        }
    }

    async function checkActiveEncounter() {
        try {
            const resp = await fetch('/api/encounters/active', {
                headers: { 'X-Session-Id': sessionId }
            });
            if (!resp.ok) return false;
            const data = await resp.json();
            if (data.active && data.encounter_id) {
                currentEncounterId = data.encounter_id;
                if (patientSelect) patientSelect.value = data.patient_id;
                if (interviewModeSelect) {
                    interviewModeSelect.value = data.mode || 'free';
                    currentInterviewMode = interviewModeSelect.value;
                    localStorage.setItem(interviewModeKey, currentInterviewMode);
                }

                await loadClinical(data.patient_id);
                await loadEncounterHistory(data.encounter_id);
                ...
                statusDiv.textContent = 'Consulta reanudada';
                return true;
            }
        } catch (e) {
            console.error('Failed to check active encounter:', e);
        }
        return false;
    }
    ...
    async function loadPatients() {
        try {
            ...
            const hasActive = await checkActiveEncounter();
            if (hasActive) return;
    ...
    const savedTtsLanguage = localStorage.getItem('tts_language') || 'es';
    ...
    if (ttsLanguageInput && !ttsLanguageInput.value.trim()) ttsLanguageInput.value = 'es';
    ...
    saveTtsConfigButton?.addEventListener('click', async () => {
        ...
        try {
            ...
            localStorage.setItem('tts_language', values.ttsLanguage || 'es');
            ...
            backendAudioState = {
                ...
                tts_language: values.ttsLanguage || 'es',
                tts_configured: Boolean(values.ttsApiKey && values.ttsVoiceId && values.ttsModelId),
            };
            pushAudioLog('CONFIG', `Guardado TTS. model=${values.ttsModelId || '-'} voice=${values.ttsVoiceId || '-'} lang=${values.ttsLanguage || 'es'} speed=${values.ttsSpeed || '-'} temp=${values.ttsTemperature || '-'} url=${values.ttsApiUrl || '-'}`, 'ok');
    ...
    function addAssistantTurn(text, timing = null, ttsPayload = null, autoplay = false) {
        const rowDiv = document.createElement('div');
    ...
    async function requestTtsPayload(text) {
        const formData = new FormData();
        formData.append('text', text);
        if (patientSelect?.value) formData.append('patient_id', patientSelect.value);
        const response = await fetch('/api/tts', {
    ...
    async function processConversation(audioBlob) {
        ...
        if (replyText) addAssistantTurn(replyText, payload.chat?.elapsed_time || null, payload.tts, true);
        renderSegueFeedback();
    ...
    async function processTextConversation(userText, sttElapsedSeconds = 0, autoSpeak = false) {
        ...
        try {
            ...
            if (audioToggle?.checked && replyText && !ttsPayload?.audio_base64) {
                try {
                    ttsPayload = await requestTtsPayload(replyText);
                } catch (ttsError) {
                    console.error('Chat TTS fallback error:', ttsError);
                    pushAudioLog('TTS', `Chat: backend TTS fallo: ${ttsError.message}`, 'error');
                }
            }
            addAssistantTurn(
                replyText,
