...
# PatientSim System Prompt Configuration
PATIENT_SIM_SYSTEM_PROMPT = """
You are simulating a patient in an Emergency Department (ED) setting.
Your character is defined by four axes:
1. **Personality**: Neutral (default), Impatient, Anxious, Suspicious, Positive, or Talkative.
2. **Language Proficiency**: Basic (A), Intermediate (B), or Advanced (C).
3. **Medical History Recall**: High (precise) or Low (forgetful).
4. **Cognitive Confusion**: Normal or High (confused/dazed).

ACT AS THE PATIENT ONLY. Do not act as the doctor.
Focus on the history taking process.
Do not provide objective clinical data (vitals, labs) unless explicitly known by the patient.
Maintain your character's traits consistently throughout the conversation.
"""
    ...
    try:
        ...
        if session_id not in chat_histories:
            chat_histories[session_id] = [
                {"role": "system", "content": PATIENT_SIM_SYSTEM_PROMPT}
            ]
        
        chat_histories[session_id].append({"role": "user", "content": message})
        
        # Force use of Ollama / Llama 3.2:1b as requested for PatientSim
        selected_model = "llama3.2:1b"
        
        # Configure client for Ollama
        # Use OLLAMA_URL from env or default to internal docker host
        ollama_url = os.getenv("OLLAMA_URL", "http://host.docker.internal:11434")
        # Ensure endpoint is v1 compatible
        if not ollama_url.endswith("/v1"):
            ollama_url = f"{ollama_url}/v1"
            
        logger.debug(f"Connecting to Ollama at {ollama_url} with model {selected_model}")
        
        # Create a transient client for this request to ensure config is fresh
        # (Or reuse an existing one if performance is critical, but here readability/correctness > perf)
        ollama_client = AsyncOpenAI(api_key="ollama", base_url=ollama_url)
        
        try:
            response = await ollama_client.chat.completions.create(
                model=selected_model,
                messages=chat_histories[session_id],
                max_tokens=250, # Increased for dialogue
            ...
            # Keep history manageable but larger than before for context
            if len(chat_histories[session_id]) > 20:
                # Keep system prompt + last 19 messages
                system_msg = chat_histories[session_id][0]
                chat_histories[session_id] = [system_msg] + chat_histories[session_id][-19:]
