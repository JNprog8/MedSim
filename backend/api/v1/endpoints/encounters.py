...
async def finish_encounter(encounter_id: str, success: bool = Body(True, embed=True)):
    return await services.encounter_service.finish_encounter(encounter_id, success)
...
@router.get("/public", response_model=List[Encounter])
async def list_public_encounters():
    """List encounters that are public/viewable"""
    return await services.encounter_service.list_public_encounters()
