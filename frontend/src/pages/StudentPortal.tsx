...
import { ArrowLeft, RefreshCw, LogIn, Search, Clock, CheckCircle, Radio } from 'lucide-react'
...
export default function StudentPortal({ view: initialView }: StudentPortalProps) {
  ...
  const [joiningEncounterId, setJoiningEncounterId] = useState('')
  ...
  const handleAdoptAndGo = async (encId: string) => {
    if (joiningEncounterId) return
    setJoiningEncounterId(encId)
    setStatusMsg('Entrando a la sesión...')
    try {
      const currentSession = localStorage.getItem(sessionIdKey) || sessionId
      const response = await fetch(`/api/encounters/${encodeURIComponent(encId)}/link`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) throw new Error('La sesión ya no está disponible.')
      ...
      setJoiningEncounterId('')
    }
  }

  // Fetch only active encounters for the search session view
  const fetchActiveEncounters = async (silent = false) => {
    if (!silent) setLoadingActive(true)
    try {
      const resp = await fetch('/api/encounters_public')
      if (!resp.ok) throw new Error('No se pudieron buscar las sesiones activas.')
      const data = await resp.json()
      const list = Array.isArray(data) ? data : (data.encounters || [])
      const activeList = list.filter((enc: Encounter) => {
        if (!enc || enc.finished_at !== null || enc.finished) return false
        return true
      })
      setActiveEncounters(activeList)
    } catch (err: any) {
      setStatusMsg(err.message || 'No se pudieron buscar las sesiones activas.')
      console.error('Error fetching active encounters', err)
    } finally {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'join' && isSearching) {
      fetchActiveEncounters()
      pollIntervalRef.current = setInterval(() => {
        fetchActiveEncounters(true)
      }, 3000)
    } else {
  ...
  const filteredEncounters = encounters
    .slice()
    .sort((a, b) => Number(Boolean(a.finished ?? (a.finished_at !== null))) - Number(Boolean(b.finished ?? (b.finished_at !== null))))
    .filter(enc => {
      ...
      <main className="max-w-4xl mx-auto px-4 mt-10">
        ...
        <div className="flex bg-slate-200/60 p-1.5 rounded-2xl mb-8">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-extrabold text-sm transition-all ${activeTab === 'join'
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-extrabold text-sm transition-all ${activeTab === 'sessions'
              ? 'bg-white text-cyan-900 shadow-sm'
              : 'text-slate-500 hover:text-slate-800'
              }`}
          >
        ...
        {activeTab === 'join' ? (
          ...
          !isSearching ? (
            ...
            <section className="bg-white border border-slate-200/60 rounded-3xl p-8 md:p-12 shadow-sm text-center flex flex-col items-center max-w-xl mx-auto">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-cyan-50 to-teal-50 border border-cyan-100/80 flex items-center justify-center mb-6 text-cyan-800 shadow-inner">
                <Search className="w-10 h-10" />
              </div>
              <h2 className="font-extrabold text-slate-900 text-2xl md:text-3xl mb-3 tracking-tight">
                Sala de Simulación
              </h2>
              <p className="text-slate-500 text-sm leading-relaxed mb-8 max-w-md">
                Presiona el botón a continuación para buscar e ingresar en tiempo real a la sesión de simulación activa iniciada por tu evaluador/a.
              </p>
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-3 px-8 py-4 bg-cyan-800 hover:bg-cyan-900 text-white font-extrabold text-base rounded-2xl shadow-lg shadow-cyan-900/20 hover:shadow-cyan-900/30 transition-all transform hover:-translate-y-0.5 active:translate-y-0 w-full sm:w-auto"
              >
                <Search className="w-5 h-5" />
                <span>Buscar sesión activa</span>
              </button>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 w-full flex items-center justify-center gap-2 text-xs text-slate-400">
                ...
                <span>Asegúrate de que tu docente o evaluador/a ya haya comenzado el caso.</span>
              </div>
            </section>
          ) : activeEncounters.length > 0 ? (
            /* State B: Tabla con la/s sesión/es activa/s */
              ...
              <div className="bg-white border border-slate-200/60 rounded-3xl p-6 md:p-8 shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-teal-50 border border-teal-100 flex items-center justify-center text-teal-700 shadow-inner">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Revisá la sesión disponible y elegí Entrar solo cuando quieras comenzar la consulta.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-auto">
                    <button
                      onClick={() => fetchActiveEncounters()}
                      disabled={loadingActive}
                      className="p-2.5 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl text-slate-600 transition-colors shadow-sm"
                      title="Actualizar sesiones activas"
                    >
                    ...
                    <button
                      ...
                      className="px-4 py-2 border border-slate-200 bg-white hover:bg-slate-50 text-xs font-bold text-slate-700 rounded-xl transition-colors shadow-sm"
                    >
                      Volver
                    </button>
                  </div>
                      ...
                      {activeEncounters.map((enc) => (
                        <tr key={enc.encounter_id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-teal-50 text-teal-700 border border-teal-200/50">
                          ...
                          <td className="px-6 py-4 text-right">
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 px-4 py-2 bg-cyan-800 hover:bg-cyan-900 text-white font-extrabold text-xs rounded-xl shadow-sm transition-all transform hover:-translate-y-0.5 active:translate-y-0"
                            >
            ...
            /* State C: Pantallita grande de esperando sesión con loading (cuando hay 0 activas) */
            <section className="bg-white border border-slate-200/60 rounded-3xl p-8 md:p-12 shadow-sm text-center flex flex-col items-center max-w-xl mx-auto animate-in fade-in duration-300">
              <div className="w-20 h-20 rounded-full bg-cyan-50 flex items-center justify-center mb-6 relative">
                <span className="absolute inset-0 rounded-full bg-cyan-200 animate-ping opacity-60" />
              ...
              <h2 className="font-extrabold text-slate-900 text-2xl mb-3">
                Esperando sesión de simulación
              </h2>
              <p className="text-slate-500 text-sm leading-relaxed mb-6 max-w-sm">
                Por favor, aguarda a que el evaluador/a inicie la sesión desde su panel de control. Las sesiones activas aparecerán aquí en cuanto comiencen.
              </p>
              <div className="flex items-center gap-2 text-xs text-cyan-900 bg-cyan-50/70 border border-cyan-100/80 px-5 py-3 rounded-full font-bold mb-6 shadow-sm">
                <RefreshCw className="w-4 h-4 animate-spin text-cyan-700" />
                <span>{statusMsg || 'Buscando simulación activa...'}</span>
              </div>
          ...
          <section className="space-y-6">
            {statusMsg && (
              <div className="bg-cyan-50 border border-cyan-100 rounded-2xl p-4 text-cyan-800 text-xs font-semibold text-center">
                {statusMsg}
              </div>
            )}
                      ...
                      pagedEncounters.map((enc) => {
                        ...
                        return (
                          <tr key={enc.encounter_id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-slate-500">{enc.evaluator_name || '-'}</td>
                            <td className="px-6 py-4">
                              <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${finished
