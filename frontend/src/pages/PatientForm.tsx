    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex gap-2">
        ...
        <button
          type="button"
          onClick={handleAdd}
          className="px-3 py-2 bg-slate-800 text-white text-xs font-bold rounded-lg hover:bg-slate-700 active:scale-95 transition-all flex items-center gap-1"
        >
          <Plus className="w-4 h-4" /> Agregar
        </button>
      </div>
      <div className="flex flex-wrap gap-1.5 mt-1.5 min-h-[32px] p-2 bg-slate-50 border border-slate-200/60 rounded-xl">
        {tags.map((tag, idx) => (
          <span
            key={idx}
            className="inline-flex items-center gap-1.5 px-3 py-1 bg-white text-slate-700 rounded-lg text-xs font-semibold border border-slate-200 shadow-sm"
          >
            {tag}
            <button
              type="button"
        ...
        {tags.length === 0 && (
          <span className="text-xs text-slate-400 italic self-center px-1">Sin elementos agregados</span>
        )}
      </div>
    </div>
  )
          ...
          <div className="p-6 overflow-y-auto">
            {activeTab === 'identity' && (
              <div className="space-y-8 animate-in fade-in duration-200">
                ...
                {/* Avatar and Voice */}
                <div>
                  <h4 className="text-xs font-extrabold uppercase text-cyan-900 tracking-wider mb-3 pb-1 border-b border-slate-100">Visual y Audio (Estático)</h4>
